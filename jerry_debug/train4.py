...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration % 2000 == 0:
            ...
            # render_pkg2 = render(scene.getTestCameras().copy()[0], gaussians, pipe, background)
        ...
        if iteration % 250 == 0:
            depth_img_trimmed = render_pkg["depth2"].detach().cpu().numpy()[0].astype(np.float32)
            ...
            axs[0].imshow(depth_img_trimmed, cmap='gray')
            axs[0].set_title('Estimate (Trimmed)')
            axs[0].axis('off')
            axs[1].imshow(depth_img, cmap='gray')
            axs[1].set_title('Estimate')
            axs[1].axis('off')
            axs[2].imshow(depth_gt, cmap='gray')
            axs[2].set_title('GT')
            axs[2].axis('off')
            plt.show()
        ...
        if iteration % 2000 == 0:
            ...
            # mesh.show()
            # mesh.export("jerry_out/voxel_mesh.ply")
            # vertices, triangles = mcubes.marching_cubes(mcubes.smooth(cube), 0.0)
            # mesh = trimesh.Trimesh(vertices, triangles)
            # mesh.show()
            # mesh.export("jerry_out/marching_cubes_mesh.ply")
