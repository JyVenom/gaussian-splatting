...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        #     savemat("jerry_out/gaussian_data.mat", {"means": gaussians.get_xyz.detach().cpu().numpy(), "rotation": gaussians.get_rotation.detach().cpu().numpy(), "scaling": gaussians.get_scaling.detach().cpu().numpy(), "opacity": gaussians.get_opacity.detach().cpu().numpy()})
        ...
        if iteration % 500 == 0:
            ...
            fig, axs = plt.subplots(3, 2, figsize=(20, 32))
        ...
        if iteration % 2000 == 0:
            ...
            for viewpoint_cam in tqdm(all_cams, desc="PCD Creation Progress", leave=False):
                ...
                remove_mask = render_pkg["mask"] < 0.99
                depth[remove_mask] = 0
                ...
                pcd, removed = pcd.remove_statistical_outlier(25, 2.5)
                ...
                for idx, norm in enumerate(norms):
                    c_vec = mat[:3, 3] - points_np[idx]
                    c_mag = np.sqrt(c_vec[0]**2 + c_vec[1]**2 + c_vec[2]**2)
                    n_mag = np.sqrt(norm[0]**2 + norm[1]**2 + norm[2]**2)
                    angle = np.degrees(np.arccos(np.dot(c_vec, norm) / (c_mag * n_mag)))

                    corrected_norm = norm
                    if angle > 90:
                        corrected_norm = corrected_norm * (-1)
                    norms_list.append(corrected_norm)
                print("Series:", time.time()-start_time)
                ...
                norm_mask = angles > 90
                norms[norm_mask] = norms[norm_mask] * (-1)
                print("Parallel:", time.time()-start_time)

                pcd.normals = o3d.utility.Vector3dVector(np.array(norms_list))
                o3d.visualization.draw_geometries([pcd], point_show_normal=True)

                pcd.normals = o3d.utility.Vector3dVector(np.array(norms))
                o3d.visualization.draw_geometries([pcd], point_show_normal=True)

                # Save data
                points = np.concatenate((points, points_np), axis=0)
                normals = np.concatenate((normals, np.array(norms_list)), axis=0)
            ...
            pcd, _ = pcd.remove_statistical_outlier(50, 1.25)
